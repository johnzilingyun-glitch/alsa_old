...
export async function saveAnalysisToHistory(type: 'market' | 'stock', data: any) {
  try {
    const response = await fetch('/api/history/save', {
      ...
      keepalive: true // Try to ensure request completes even if page unloads
    });
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      console.warn('Save analysis API rejected:', errorData);
    }
  } catch (err) {
    // Non-critical background task: downgrade console error to warn and ignore "Load failed" (Safari abort)
    if (err instanceof Error && (err.message.includes('Load failed') || err.name === 'AbortError')) {
      return; 
    }
    console.warn('Background save to history task failed:', err);
  }
}
