...
export function enforceGroundTruth(analysis: StockAnalysis, realtimeData: any): void {
  ...
  // Fundamental Metrics Ground Truth
  if (realtimeData.marketCap != null) analysis.stockInfo.marketCap = Number(realtimeData.marketCap);
  if (realtimeData.pe != null) analysis.stockInfo.pe = Number(realtimeData.pe);
  if (realtimeData.peForward != null) analysis.stockInfo.peForward = Number(realtimeData.peForward);
  if (realtimeData.pb != null) analysis.stockInfo.pb = Number(realtimeData.pb);
  if (realtimeData.eps != null) analysis.stockInfo.eps = Number(realtimeData.eps);
  if (realtimeData.epsForward != null) analysis.stockInfo.epsForward = Number(realtimeData.epsForward);
  if (realtimeData.roe != null) analysis.stockInfo.roe = Number(realtimeData.roe);
  if (realtimeData.revenueGrowth != null) analysis.stockInfo.revenueGrowth = Number(realtimeData.revenueGrowth);
  if (realtimeData.dividendYield != null) analysis.stockInfo.dividendYield = Number(realtimeData.dividendYield);
  if (realtimeData.debtToEquity != null) analysis.stockInfo.debtToEquity = Number(realtimeData.debtToEquity);
  if (realtimeData.volume != null) analysis.stockInfo.volume = Number(realtimeData.volume);
