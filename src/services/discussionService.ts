...
function compactMessages(messages: AgentMessage[], currentRound: number): AgentMessage[] {
  return messages.map((msg) => {
    // Keep full context for the immediately preceding round or key roles
    if (msg.round === currentRound - 1 || msg.role === 'Deep Research Specialist' || msg.role === 'Chief Strategist') {
      return msg;
    }
    // Deep research mode needs more context preservation
    const limit = 1200; 
    if (msg.content.length > limit) {
      return {
        ...msg,
        content: msg.content.substring(0, limit) + '... [Content compacted for token efficiency]'
      };
    }
    return msg;
  });
...
const DEEP_MODEL = "gemini-3.1-pro-preview";           // Latest flagship for Deep Research mode
...
function isLowQualityExpertContent(role: AgentRole, content: string): boolean {
  ...
  // Minimum 300 characters for a professional analysis to ensure depth as requested
...
export async function startMultiRoundDiscussion(
  ...
  for (const round of expandedTopology) {
    ...
    const callExpert = async (role: AgentRole): Promise<ExpertOutput | null> => {
      ...
      const compactedContext = compactMessages(allMessages, roundIdx);
      ...
      const invokeExpert = async (inputPrompt: string) => {
        // Determine model based on role criticality and analysis level
        let model = DRAFTING_MODEL;
        ...
        // Standard level uses FLASH for everything to keep it fast and free
        if (level === 'standard' || level === 'quick') {
          model = 'gemini-1.5-flash';
        }

        // Critical roles get PRO in Deep mode. Others use FLASH to preserve quota.
        if (level === 'deep') {
          const ELITE_ROLES: Set<AgentRole> = new Set([
            'Chief Strategist',
            'Professional Reviewer',
            'Deep Research Specialist',
            'Fundamental Analyst'
          ...
          if (ELITE_ROLES.has(role)) {
            model = DEEP_MODEL; // 3.1 Pro
          } else {
            model = 'gemini-1.5-flash'; // High quota fallback
          }
        }
        ...
        if (needsSearch) {
          finalPrompt += `\n\n**CRITICAL INSTRUCTION**: You are equipped with Google Search tools. You MUST use them to fetch real-time data ONLY. Regardless of whether you find the data or not, YOUR ENTIRE RESPONSE MUST BE VALID JSON MATCHING THIS SCHEMA: ${JSON.stringify(schema)}\n\nDo not output markdown text directly. Return ONLY a JSON string. Use 'content' field in JSON to output your analytical text.`;
        }
        ...
        return generateAndParseJsonWithRetry<any>(ai, {
          ...
          transportRetries: 3, // Increased from 2 for better resilience
          baseDelayMs: 4000,   // Increased from 3000
          responseMimeType: "application/json",
