...
export const Header = memo(function Header({ onSearch, onResetToHome, onTriggerDailyReport, onOpenHistory, onFetchAdminData }: HeaderProps) {
  ...
  const suggestionsCache = useRef<Record<string, any[]>>({});
  ...
  useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      ...
      // Check cache first
      const cacheKey = `${market}:${trimmed.toLowerCase()}`;
      if (suggestionsCache.current[cacheKey]) {
        setSuggestions(suggestionsCache.current[cacheKey]);
        setShowSuggestions(suggestionsCache.current[cacheKey].length > 0);
        setSelectedIndex(-1);
        return;
      }
      ...
      try {
        ...
        const res = await fetch(`/api/stock/suggest?${params.toString()}&t=${Date.now()}`, { signal: controller.signal });
        if (res.ok) {
          const contentType = res.headers.get('content-type');
          if (contentType && contentType.includes('application/json')) {
            const data = await res.json();
            suggestionsCache.current[cacheKey] = data;
            setSuggestions(data);
        ...
        const isAbortError = e.name === 'AbortError' || controller.signal.aborted;
        const isLoadFailed = e.message && (
          e.message.toLowerCase().includes('load failed') || 
          e.message.toLowerCase().includes('failed to fetch') ||
          e.message.toLowerCase().includes('aborted')
        );

        if (!isAbortError && !isLoadFailed) {
